// Package tape's accessible mode provides a sequential, line-by-line
// interactive interface for adjusting narration timing. It uses no
// alternate screen buffer, no full-screen redraws, and no spatial layout
// — just standard input/output that screen readers can follow.
...
func RunAccessible(
	...
	for {
		...
		case "+", "right", "l":
			...
			newPause := NudgePause(steps[cursor], nudgeStep)
...
func currentPause(step Step) float64 {
	if step.Pause != nil {
		return *step.Pause
	}
	return DefaultPause
...
func announceStep(w io.Writer, steps []Step, index int, nudgeStep float64) {
	step := steps[index]
	startTime := StepStartTime(steps, index)
